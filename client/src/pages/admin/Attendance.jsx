...
import { Alert, ApiError, EmptyState, Field, Modal, NameCell, PageHeader, PageLoader } from '../../components/ui';
...
export default function Attendance() {
  ...
  const setRec = (studentId, key, value) =>
    setRecords((prev) => ({ ...prev, [studentId]: { ...prev[studentId], [key]: value } }));

  const studentsForAssignment = (classSubjectId) => {
    const asg = assignments.find((a) => a.id === classSubjectId);
    if (!asg) return [];
  ...
  const createSession = async (e) => {
    ...
    try {
      ...
      const classStudents = studentsForAssignment(sessionForm.classSubjectId);
      const initRecords = {};
      classStudents.forEach((st) => {
        initRecords[st._id] = { status: 'present', remarks: '' };
      });
      ...
      setSuccess('Attendance session created — now mark attendance below');
      setShowCreate(false);
    } catch (err) {
      setError2(err);
    } finally {
  ...
  return (
    <div>
      <PageHeader
        title="Attendance"
        subtitle={`${assignments.length} class assignments available`}
      />
        ...
        <div className="card">
          <div className="card-body">
            <EmptyState
              ...
              message="Create class-subject assignments first, then take attendance."
            />
          </div>
        </div>
      ) : (
        <div className="card">
          <div className="card-header"><h3>Class assignments</h3></div>
          <div className="card-body">
                  ...
                  return (
                    <tr key={sid}>
                      ...
                      <td style={{ width: 180 }}>
                        <select
                          className="select"
                          value={records[sid].status}
                          onChange={(e) => setRec(sid, 'status', e.target.value)}
                        >
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Start Attendance Session">
        <form onSubmit={createSession}>
          ...
          <Field label="Remarks">
            <input className="input" value={sessionForm.remarks} onChange={setS('remarks')} placeholder="Optional note" />
          </Field>
